...
export async function POST(req: NextRequest) {
  ...
  const storagePath = `office/letterhead-${Date.now()}.${ext}`;
  ...
  if (uploadError) {
    return NextResponse.json({ error: `فشل رفع ورق الشركة: ${uploadError.message}` }, { status: 500 });
  }
  ...
  const settings = existing
    ? await prisma.officeSettings.update({
        ...
        data: { letterheadUrl: publicUrlData.publicUrl },
      })
    : await prisma.officeSettings.create({ data: { letterheadUrl: publicUrlData.publicUrl } });
