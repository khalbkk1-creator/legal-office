...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const allowed = await hasAccountingPermission(user.id, user.role, "journalReverse");
  if (!allowed) {
    return NextResponse.json({ error: accountingPermissionError("journalReverse") }, { status: 403 });
  }
