...
export default function NewQuotePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    if (!res.ok) {
      setError("تعذر إنشاء عرض السعر، تحقق من الحقول المطلوبة.");
      return;
    }
    router.push("/quotes");
  }
  ...
  return (
    <div className="max-w-lg">
      <h1 className="text-2xl font-bold text-ink mb-6">عرض سعر جديد</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="مثال: أتعاب ترافع أمام المحكمة"
            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">صالح حتى (اختياري)</label>
            <input
              type="date"
