...
function hash(code: string, clientId: string) {
  return crypto.createHmac("sha256", process.env.NEXTAUTH_SECRET || "otp").update(`${clientId}:${code}`).digest("hex");
}
...
export async function issueOtp(client: { id: string; email: string | null; phone: string | null }, purpose: OtpPurpose, officeName: string, requestedChannel?: string) {
  const since = new Date(Date.now() - 15 * 60000);
  const recent = await prisma.portalOtp.count({ where: { clientId: client.id, purpose, createdAt: { gte: since } } });
  ...
  await prisma.portalOtp.updateMany({ where: { clientId: client.id, purpose, consumedAt: null }, data: { consumedAt: new Date() } });
  await prisma.portalOtp.create({ data: { clientId: client.id, purpose, channel, codeHash: hash(code, client.id), expiresAt: new Date(Date.now() + OTP_TTL_MIN * 60000) } });
  ...
  await logPortalAccess(client.id, "OTP_SENT", { detail: `${label} عبر ${channel === "EMAIL" ? "البريد" : "SMS"}` });
  const masked = channel === "EMAIL" ? maskEmail(client.email!) : maskPhone(client.phone!);
  return { ok: true as const, channel, masked };
}

export async function verifyOtp(clientId: string, purpose: OtpPurpose, code: string) {
  const otp = await prisma.portalOtp.findFirst({ where: { clientId, purpose, consumedAt: null }, orderBy: { createdAt: "desc" } });
  ...
  if (otp.codeHash !== hash(code.trim(), clientId)) {
    await prisma.portalOtp.update({ where: { id: otp.id }, data: { attempts: { increment: 1 } } });
    await logPortalAccess(clientId, "OTP_FAILED", { detail: `محاولة ${otp.attempts + 1}` });
